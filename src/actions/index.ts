...
import { RESEND_API_KEY, CONTACT_TO } from "astro:env/server";
...
const getMessage = (name: string, email: string, message: string, phone?: string): string => {
    let result = `${message}\n\n${email}`;
    if (name) {
        result += `\n${name}`;
    }
...
export const server = {
    ...
    contact: defineAction({
        ...
        handler: async (input) => {
            ...
            const recipientEmail = CONTACT_TO || "ko.alessio@gmail.com";
